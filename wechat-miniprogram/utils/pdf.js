...
function pt(mm) { return mm * PT_PER_MM }
...
function sanitizeFilename(value, fallback) {
  return String(value || fallback || 'String_Art_Nail_Template')
    .replace(/[\\/:*?"<>|\x00-\x1f]/g, '_')
    .replace(/\s+/g, ' ')
    .trim()
...
function chooseTiling(templateSizeMm) {
  ...
  for (let columns = 1; columns <= 12; columns += 1) {
    for (let rows = 1; rows <= 12; rows += 1) {
      if (templateSizeMm / columns > USABLE_WIDTH_MM || templateSizeMm / rows > USABLE_HEIGHT_MM) continue
...
function buildPage(options, tiling, row, column) {
  ...
  addText(commands, 'Page ' + (row * tiling.columns + column + 1) + '/' + tiling.pages +
    ' | Column ' + (column + 1) + '/' + tiling.columns + ' | Row ' + (row + 1) + '/' + tiling.rows,
  pt(2.2), PAGE_WIDTH_PT - pt(10), PAGE_HEIGHT_PT - pt(8.5), 'right')
  ...
  commands.push('0.35 w', templateTopLeft.x.toFixed(3) + ' ' + (templateTopLeft.y - pt(templateSizeMm)).toFixed(3) + ' ' + pt(templateSizeMm).toFixed(3) + ' ' + pt(templateSizeMm).toFixed(3) + ' re S')
  ...
  for (let index = 0; index < nails; index += 1) {
    ...
    const label = pagePoint(
      ...
      tileLeft, tileTop, marginX, marginY
    )
  ...
  const rulerY = pt(6)
  commands.push('0.5 w', rulerStart.toFixed(3) + ' ' + rulerY.toFixed(3) + ' m ' + (rulerStart + pt(100)).toFixed(3) + ' ' + rulerY.toFixed(3) + ' l S')
...
function generateNailTemplate(input) {
  ...
  streams.forEach((stream, index) => {
    ...
    objects.push('<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ' + PAGE_WIDTH_PT + ' ' + PAGE_HEIGHT_PT +
