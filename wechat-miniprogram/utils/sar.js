...
class ByteReader {
  ...
  require(count) {
    if (count < 0 || this.offset + count > this.bytes.length) throw new Error('SAR 文件意外结束')
  }
...
function validateProject(project, miniProgramLimits) {
  ...
  project.sequence.forEach((value) => {
    if (!Number.isInteger(value) || value < 0 || value >= project.params.nails) throw new Error('SAR 包含非法钉号')
  })
...
function decodeSar(input, options) {
  ...
  if (magic !== 'SAR2' && magic !== 'SAR3' && magic !== 'SAR4') throw new Error('未知 SAR magic: ' + magic)
  const project = {
    magic,
    name: reader.readUTF(),
  ...
  if (magic === 'SAR4') {
    ...
    if (thumbnailLength < 0 || thumbnailLength > MAX_THUMBNAIL_BYTES) throw new Error('SAR 缩略图长度无效')
